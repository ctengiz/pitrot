...
$(function() {
    $("sstable").tablesorter({
        ...
        widgets : [ "uitheme", "filter", "zebra"], //zebra ??
...
$(document).ready(function() {
    function popajax(act, new_val, row_id, new_val_text) {
        $.ajax({
        type: "POST",
        url: '/issue_popup',
        data: {
        ...
        success: function(data, textStatus, jqXHR) {
            if (data.success == 1) {
                if (new_val_text) {
                    $("#" + row_id).children('[data-field="' + act + '"]').html(new_val_text);
                } else {
                    $("#" + row_id).children('[data-field="' + act + '"]').text(new_val);
                }
            } else {
                alert(data.message);
            }
    ...
    $("#atable tr").contextMenu({
        menuSelector: "#contextMenu",
        menuSelected: function (invokedOn, selectedMenu) {
            var act = selectedMenu.data("act");
			var project_id = invokedOn.data('project_id');
			var issue_id = invokedOn.attr("id");


            if (act) {
                if ((act == 'dt_plan') || (act == 'dt_due')) {
                    var cur_val = invokedOn.children('[data-field="' + act + '"]').text();
                    $("#popup-" + act).val(cur_val);
                    $("#popup-" + act).data('row_id', issue_id);
                    $("#modal-" + act).modal();
                } else

                if (act == 'usr_id_assigned') {
                    var cur_val_text = invokedOn.children('[data-field="' + act + '"]').text();
                    var cur_val = invokedOn.data('usr_id_assigned');

                    var usr_data;
                    $.ajaxSetup({async: false});
                    $.post("/ajax/usr", {"project_id": project_id}, "json").done(function(data){
                        usr_data = eval(data);
                    });
                    $.ajaxSetup({async: true});

                    $("#popup-" + act).find('option').remove().end();

                    for(var k in usr_data) {
                        $("#popup-" + act).append('<option value=' + usr_data[k].id + '>' + usr_data[k].code + '</option>');
                    };
                ...
                if (act == "project_id") {
                    ...
                    var cur_val_text = invokedOn.children('[data-field="' + act + '"]').text();
                    var cur_val = invokedOn.data('project_id');
                ...
                if (act == "edit") {
                    //window.location = "/issue/" + issue_id;
                    window.open("/issue/" + issue_id, '_blank');
                } else
                ...
                if (act == "del") {
                    ...
                    var new_val = selectedMenu.data('val');
                    var new_val_text = selectedMenu.text();
                    var row_id = invokedOn.attr("id");
                    popajax(act, new_val, row_id, new_val_text);
                }
            }
        }
    });

    $("button[data-act]").click(function () {
        var act = $(this).data("act");
        var new_val = null;
        if (act) {
            var new_val = $("#popup-" + act).val();

            var new_val_text = null;
            if ((act == 'usr_id_assigned') || (act == 'project_id')) {
                var new_val_text = '<a href="/user/' + new_val + '">'
                        +  $("#popup-" + act + " option:selected").text()
                        + '</a>';
            }
            var row_id = $("#popup-" + act).data('row_id');
